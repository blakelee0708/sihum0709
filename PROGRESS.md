...
**③ 만세력 검증** (규칙 3) — ⬅ **이제 이게 첫 번째 할 일입니다**

`test/saju-output.md`에 검증 사례 10건의 계산 결과를 표로 뽑아 두었습니다.
다른 만세력 서비스와 대조하신 뒤, `lib/saju/calculate.test.ts` 위쪽
`EXPECTED` 표의 TODO 자리에 정답을 채워 넣으시면 테스트가 실제 검증으로 바뀝니다.

절기 계산은 이미 교차 검증했습니다. 2024년 입춘 17:27, 2025년 입춘 2월 3일 23:10,
1988년 입춘 23:43이 공표값과 일치합니다. 일주 기준일도 `1936-02-12 = 갑자일`로
확인했습니다. 그래도 대조가 필요한 이유는 아래 2번의 판단 때문입니다.
...
**① 2026년 입춘 시각이 PRD 예시와 다릅니다**

PRD 4.1.1 예시 JSON은 `2026-02-04 05:46`, skyfield 계산값은 `05:02`입니다.
같은 방식으로 계산한 2024·2025·1988년 값이 공표 만세력과 정확히 일치하므로
계산 쪽을 택했습니다. PRD 예시는 형식 설명용 숫자로 봤습니다.

**② PRD 8.7의 삼성전자 일간이 계산값과 다릅니다**

PRD 8.7 출력 예시는 삼성전자(1969-01-13) 기업 일간을 **경금(庚金)**으로 적었으나,
PRD 4.1.2가 준 기준일로 계산하면 **무토(戊土)**가 나옵니다.
기준일 자체는 `1936-02-12 = 갑자일`로 교차 검증되므로 PRD 예시가 설명용 숫자인
쪽으로 보고 진행했습니다. 3번 항목 대조 때 함께 확인해 주시면 좋겠습니다.
