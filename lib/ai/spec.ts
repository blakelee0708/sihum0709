...
const W_WEEK_PLAN: SectionSpec = {
  ...
  structure:
    ...
    '(2) 그 점수에 맞춘 학습량과 종류를 씁니다. 하루당 80자 이상입니다.',
  minChars: 600,
  maxChars: 800,
  basis: '일별 일진 × 사용자 일간',
}
...
const W_CAUTIONS: SectionSpec = {
  ...
  structure:
    ...
    '(2) 언제 어떤 모습으로 나타나는지 (3) 무엇으로 막는지를 씁니다. 항목당 100자 이상입니다.',
  minChars: 300,
  maxChars: 400,
...
const W_REMAINING_USE: SectionSpec = {
  ...
  structure:
    ...
    '문단마다 남은 기간의 어느 구간에 배치할지와 그 순서인 이유를 약한 오행 점수로 설명합니다.',
  minChars: 300,
  maxChars: 400,
  basis: '약한 오행 보완 순서',
}
...
const W_SEAT: SectionSpec = {
  ...
  structure:
    '문단 2개. 첫 문단은 약한 오행이 몇 점이고 그 방위가 어디인지, 좌석에서 할 수 있는 조정. ' +
...
const W_AFTER: SectionSpec = {
  ...
  structure:
    '문단 3개. 첫 문단은 방식 궁합 점수를 높은 순으로 짚고 무엇이 상대적으로 나은지. ' +
    '둘째 문단은 월별 흐름에서 점수가 높은 달을 근거로 다음 기회를 언제로 볼지. ' +
...
const W_NOW_THREE: SectionSpec = {
  ...
  structure:
    '정확히 세 문장. 문장마다 무엇을 하는지와 왜 하는지를 함께 담아 85자 이상으로 씁니다. ' +
...
const I_ANSWER_TYPE: SectionSpec = {
  ...
  structure:
    ...
    '항목마다 강한 오행 또는 약한 오행의 점수를 근거로 대고 면접장에서의 행동까지 씁니다.',
  minChars: 380,
  maxChars: 480,
  basis: '강한 오행 + 약한 오행',
}
...
const I_QUESTIONS: SectionSpec = {
  ...
  structure:
    '유형 4개. 약점에서 역산한 유형 3개와 강점이 드러날 유형 1개. ' +
    '유형마다 (1) 어느 오행·십신이 몇 점이라 이 유형이 나오는지 ' +
    '(2) 실제로 어떤 형태의 질문으로 들어오는지 (3) 어떻게 답할지를 씁니다. 유형당 150자 이상입니다.',
  minChars: 550,
  maxChars: 700,
  basis: '약한 오행 + 십신 관계',
}
...
const I_OUTFIT: SectionSpec = {
  ...
  structure:
    '문단 2개. 첫 문단은 복장 — 약한 오행의 색을 어디에 넣을지, 면접 복장 기본은 지키면서. ' +
    '둘째 문단은 소지품 — 무엇을 챙기고 그것이 어떤 오행을 채우는지. 섹션 10과 겹치지 않게.',
  minChars: 250,
  maxChars: 330,
  basis: '약한 오행 색상',
}
