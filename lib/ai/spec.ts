...
export type SectionSource = 'calc' | 'ai' | 'ai+search' | 'calc+fragment+ai'
...
const W_WEEK_PLAN: SectionSpec = {
  ...
  source: 'ai',
  brief:
    'weekFlow의 날짜별 점수를 근거로 D-7부터 당일까지 하루씩 무엇을 할지 쓴다. 점수가 낮은 날에 무리한 분량을 배치하지 않는다.',
}
...
const W_CAUTIONS: SectionSpec = {
  ...
  title: '놓치기 쉬운 3가지',
  source: 'ai',
  brief: '약한 오행에서 도출한 실수 패턴 3가지와 각각의 대응을 쓴다.',
}
...
const W_NOW_THREE: SectionSpec = {
  ...
  title: '지금 바로 할 3가지',
  source: 'ai',
  brief: '정확히 세 문장으로 쓴다. 지금 즉시 할 수 있는 행동만 쓴다.',
  highlight: true,
}
...
const I_QUESTIONS: SectionSpec = {
  ...
  source: 'ai',
  brief:
    '기출 질문을 옮기지 않는다. 사주가 알려주는 약점에서 질문 유형을 역산해 유형 3-4개를 쓰고, 각 유형마다 들어올 수 있는 형태와 대응을 붙인다. 강점이 드러날 유형도 함께 넣어 균형을 맞춘다.',
}
