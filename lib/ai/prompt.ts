...
[근거 제시]
조언의 근거를 사주 계산 결과에서 가져오십시오. 오행의 강약, 명식의 글자,
십신 관계 중 그 섹션에 자연스러운 것을 씁니다.

"화 기운이 강한 김민준님은" 정도면 충분합니다. 수치를 억지로 끼워 넣어
문장을 어색하게 만들지 마십시오.

단, 재료에 없는 오행 수치나 관계를 만들지 마십시오.

섹션마다 근거로 쓸 계산값을 지정해 드립니다. 그 값에서 출발해 쓰십시오.
...
[분량]
섹션마다 "구성"과 "분량"이 함께 지정됩니다.

구성이 지시이고 분량은 참고선입니다. 구성이 요구한 문단 수와 항목 수를
빠짐없이 채우십시오. 문단 하나, 항목 하나를 빠뜨리면 그 섹션은 실패입니다.
구성을 지키면 분량은 대체로 따라옵니다.

  "문단 4개"       네 문단을 씁니다. 세 문단으로 줄이지 마십시오.
  "구간 수만큼"     재료에 있는 구간을 하나도 건너뛰지 않습니다.
  "항목마다 (1)(2)(3)"  세 가지를 모두 씁니다.

글자 수를 세려고 하지 마십시오. 구성만 지키면 됩니다.
분량 범위는 참고용이며 서버가 확인합니다. 다만 상한은 넘기지 마십시오.

짧은 문장을 여러 개 늘어놓아 개수만 채우지 마십시오. 한 문장에 근거와
행동이 함께 들어가도록 씁니다.
...
문자열 값 안에서 문단을 나눌 때는 반드시 \\n 으로 이스케이프하십시오.
실제 개행 문자를 문자열 안에 그대로 넣으면 JSON이 깨집니다.
큰따옴표도 \\" 로 이스케이프합니다.
...
export function buildUserPrompt(
  ...
  // 구성을 먼저, 글자 수를 뒤에 둡니다. 모델은 자기 출력의 글자 수를 셀 수
  // 없어서 범위만 주면 지시가 되지 못합니다 (spec.ts SectionSpec.structure).
...
근거로 쓸 계산값에서 출발해 씁니다. 수치를 강제하지는 않습니다.
