 ...
 *
 * ── 지장간을 반영하지 않는 이유 (확정 규칙) ──
 *
 * 지지 속에 숨은 천간(지장간)을 점수에 넣지 않고, 지지의 대표 오행만 셉니다.
 *
 * 명리에서 더 정밀한 방식은 지장간까지 보는 것이지만, 이 서비스가 오행 강약을
 * 쓰는 곳은 문장 조각을 고르는 자리 하나뿐입니다(강한 오행 5종, 약한 오행 5종).
 * 정밀도를 올려도 사용자가 받는 결과는 조각 선택이 가끔 바뀌는 정도이고,
 * 대신 계산이 무거워지고 "왜 이 유형이 나왔는지" 설명하기 어려워집니다.
 *
 * 그래서 PRD 5.2의 가중치 표를 그대로 씁니다. 바꾸려면 이 파일의
 * getElementScores 하나만 고치면 됩니다.
 */
